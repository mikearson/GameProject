...
public class TileMapObj extends TileMap {
    ...
    public TileMapObj(String data, Sprite sprite, int width, int height, int blockWidth, int blockHeight) {
        ...
        for (int i = 0; i < (width * height); i++) {
            ...
            if (temp != 0) {
                // if(temp == 41){
                // ObjBlock[i] = new WaterBlock(sprite.getSprite((int) (temp - 1) % 40, (int)
                // (temp -1) / 36),
                // new Vector2f((int) (i % width) * blockWidth,(int) (i / height) *
                // blockHeight),
                // blockWidth, blockHeight);
                // } else {
                ObjBlock[i] = new ObjBlock(sprite.getSprite((int) (temp - 1) % 40, (int) (temp - 1) / 36),
                        new Vector2f((int) (i % width) * blockWidth, (int) (i / height) * blockHeight), blockWidth,
                        blockHeight);
                // }
            }
